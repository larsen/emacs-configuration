...
(defun larsen/calendar-iso-day-to-gregorian (week-number week-day)
...
(defun larsen/gregorian-date-as-string (date)
  (cl-destructuring-bind (month day year) date
  (format "%4d-%02d-%02d" year month day)))

(defun larsen/current-week-number ()
  "Return ISO8601 week number for today."
  (format-time-string "%V" (current-time)))

(defvar day-properties-string ":PROPERTIES:\n:COLUMNS: %25ITEM %TAGS %PRIORITY %TODO\n:END:")

(defun larsen/week-heading (week-number)
  "Return a string representing the heading for WEEK-NUMBER todo entries subtree."
  (let ((week-begin-date (larsen/calendar-iso-day-to-gregorian week-number 1))
        (week-end-date (larsen/calendar-iso-day-to-gregorian week-number 5)))
    (format "Week %d (%s - %s)"
            week-number
            (larsen/gregorian-date-as-string week-begin-date)
            (larsen/gregorian-date-as-string week-end-date))))

(defun larsen/today-heading ()
...
(defun larsen/insert-current-week-item (week-number)
  "Insert 1-week-worth structure for entering todo entries for WEEK-NUMBER."
  (interactive "P")
  (insert (format "* %s\n" (larsen/week-heading week-number)))
...
(defun larsen/org-set-item-deadline (week-number)
  (let ((week-end-date (larsen/calendar-iso-day-to-gregorian week-number 5)))
    (org-deadline nil (larsen/gregorian-date-as-string week-end-date))))

(defun larsen/org-set-tree-deadline (week-number)
  (interactive "P")
  (org-map-entries (lambda () (larsen/org-set-item-deadline week-number)) nil 'tree))
...
(defun larsen/find-today-heading ()
  (let ((m (org-find-olp `(,(larsen/week-heading
                             (string-to-number (larsen/current-week-number)))
                           ,(larsen/today-heading))
...
(defun larsen/org-jump-to-today ()
...
(defun larsen/org-download-screenshot ()
...
(global-set-key (kbd "<C-print>") 'larsen/org-download-screenshot)
